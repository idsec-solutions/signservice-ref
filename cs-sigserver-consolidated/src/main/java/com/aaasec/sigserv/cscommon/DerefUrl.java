...
public class DerefUrl {
    ...
    static public byte[] getBytes(String urlStr, SslSecurityPolicy policy) {
        try {
            URL url = new URL(urlStr);
            return getBytes(url, policy);
        } catch (MalformedURLException ex) {
            Logger.getLogger(DerefUrl.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }
