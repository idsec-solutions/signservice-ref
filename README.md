...
### 2.2 Configuration of CRL update cycle and periodic updates

The default update cycle for CRL publishing and other periodic updates such as metadata publication is
1 hour between updates.

This can be altered by setting the environment variable `DAEMON_CYCLE_SECONDS` to the number of seconds between updates.
The following setting in docker run sets the update cycle to once a day:

> -e "DAEMON_CYCLE_SECONDS=86400"

**Important note:**
Make sure that the CRL publication cycle is longer than this daemon update cycle. If this condition is not met, **the CRL
will expire before a new CRL is published**, preventing signatures from being validated between updates.
